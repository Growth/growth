...
/***/ function(module, exports, __webpack_require__) {
	...
	function get(name) {
	    return (0, _utils.isString)(name) ? findOrCreate(name) : name;
	}
	...
	function namespace(identifier) {
	    ...
	    for (var fn in Tag) {
	        Namespace[fn] = (0, _utils.wrap)(Tag[fn], handleNamespace(identifier));
	    }
	...
	function handleNamespace(namespace) {
	    return function (item, name) {
	        return [name ? item : (0, _utils.addNamespace)(item, namespace), name ? (0, _utils.addNamespace)(name, namespace) : name];
	    };
	}
...
/***/ function(module, exports) {
	...
	function delayApply(callback, context) {
	    for (var _len = arguments.length, args = Array(_len > 2 ? _len - 2 : 0), _key = 2; _key < _len; _key++) {
	        args[_key - 2] = arguments[_key];
	    }

	    delay(function () {
	        return callback.apply.apply(callback, [context].concat(args));
	    });
	}

	function delayCall(callback, context) {
	...
	function wrap(wrapped, wrapper) {
	    return function () {
	        return wrapped.apply(wrapped, wrapper.apply(undefined, arguments));
	    };
	}
	...
	function stringToArray(string) {
	    ...
	    return string.split(separator).map(function (str) {
	        return str.trim();
	    });
	}
	...
	function spawnTask(generator) {
	    var task = generator(function () {
	        return task.next.apply(task, arguments);
	    });
	    task.next();
...
/***/ function(module, exports, __webpack_require__) {
	...
	function listen(emitter, eventName, listener) {
	    var listeners = listenersFor(emitter, eventName);
	    if (listeners.indexOf(listener) === -1) {
	        listenersFor(emitter, eventName).push(listener);
	    }
	}

	function emit(emitter, eventName) {
	    var listeners = listenersFor(emitter, eventName);
	...
	function dispatch(emitters, eventName) {
	    for (var _len2 = arguments.length, args = Array(_len2 > 2 ? _len2 - 2 : 0), _key2 = 2; _key2 < _len2; _key2++) {
	        args[_key2 - 2] = arguments[_key2];
	    }
	    ...
	    try {
	        for (var _iterator2 = emitters[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
	            ...
	            emit.apply(undefined, [emitter, eventName].concat(args));
	        }
	    } catch (err) {
	        _didIteratorError2 = true;
	        _iteratorError2 = err;
	...
	function forwardTo(emitter, eventName) {
	    return function () {
	        ...
	        return emit.apply(undefined, [emitter, eventName].concat(args));
	    };
	}

	function removeListener(emitter, eventName, listener) {
	    (0, _utils.remove)(listenersFor(emitter, eventName), listener);
	}

	function clearEmitter(emitter) {
	    var listenersBox = getListenersRegister(emitter);
	    for (var eventName in listenersBox) {
	...
	function listenersFor(emitter, eventName) {
	    var listenersRegister = getListenersRegister(emitter);
